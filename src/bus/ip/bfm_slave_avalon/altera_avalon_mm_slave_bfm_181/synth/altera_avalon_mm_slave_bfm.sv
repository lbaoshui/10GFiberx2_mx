...
// $File: //acds/rel/18.1/ip/sopc/components/verification/altera_avalon_mm_slave_bfm/altera_avalon_mm_slave_bfm.sv $
// $File: //acds/rel/18.1/ip/sopc/components/verification/altera_avalon_mm_slave_bfm/altera_avalon_mm_slave_bfm.sv $
// $Revision: #1 $
// $Date: 2018/07/29 $
// $Author: psgswbuild $
...
module altera_avalon_mm_slave_bfm(
   ...
   parameter AV_WAITREQUEST_ALLOWANCE   = 0; // Controls the waitrequest allowance value
   ...
   logic               next_waitrequest_value = 1'b1;
...
BFM will reset the wait cycles to 1");
   ...
   // Sets the value of waitrequest on the next rising clock edge.
   // Should only be used when AV_WAITREQUEST_ALLOWANCE > 0.
   function automatic void set_waitrequest(
      logic waitrequest_value
   );
      next_waitrequest_value = waitrequest_value;
   endfunction
   ...
   function automatic void __hello();
      ...
      print(VERBOSITY_INFO, message);            
      $sformat(message, "%m: -   $Date: 2018/07/29 $");
      print(VERBOSITY_INFO, message);
   ...
   generate if (AV_WAITREQUEST_ALLOWANCE > 0) begin : waitrequest_update
      always @(posedge clk) begin
         avs_waitrequest <= next_waitrequest_value;
      end
   end endgenerate
